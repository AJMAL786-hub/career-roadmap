...
export default function FloatingLines({
  ...
  // The props below are passed as inline array/object literals whose identity
  // changes on every render. Capturing them in refs lets the WebGL renderer be
  // created exactly ONCE, avoiding the forceContextLoss()/recreate churn (and
  // resulting flicker) that would otherwise happen on every parent re-render.
  ...
  const getLineCount = waveType => {
    const { lineCount, enabledWaves } = dynamicRef.current;
  ...
  const getLineDistance = waveType => {
    const { lineDistance, enabledWaves } = dynamicRef.current;
  ...
  const { enabledWaves: ew, topWavePosition: twp, middleWavePosition: mwp, bottomWavePosition: bwp } = dynamicRef.current;
  ...
  }, [animationSpeed, interactive, bendRadius, bendStrength, mouseDamping, parallax, parallaxStrength, bottomWavePosition?.y]);
